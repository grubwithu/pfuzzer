...
size_t TracePC::GetTotalPCCoverage(struct CoverageInfo &CI) {
  return CI.ObservedPCs.size();
}
...
void TracePC::HandleInline8bitCountersInit(uint8_t *Start, uint8_t *Stop) {
  ...
  size_t NumFullPages = AlignedStop > AlignedStart ? (AlignedStop - AlignedStart) / PageSize() : 0;
...
void TracePC::UpdateObservedPCs(struct CoverageInfo &CI) {
  std::vector<uintptr_t> CoveredFuncs;
  auto ObservePC = [&](const PCTableEntry *TE) {
    if (CI.ObservedPCs.insert(TE).second && DoPrintNewPCs) {
  ...
  auto Observe = [&](const PCTableEntry *TE) {
    if (PcIsFuncEntry(TE))
      if (++CI.ObservedFuncs[TE->PC] == 1 && NumPrintNewFuncs)
...
void TracePC::GetSeedTrace() {
  CurrentObservedPCs.clear();
  // size_t Dont = 0, Exist = 0;
  if (NumPCsInPCTables) {
    if (NumInline8bitCounters == NumPCsInPCTables) {
      for (size_t i = 0; i < NumModules; i++) {
        auto &M = Modules[i];
        ...
        for (size_t r = 0; r < M.NumRegions; r++) {
          ...
          for (uint8_t *P = R.Start; P < R.Stop; P++)
            if (*P) {
              CurrentObservedPCs.push_back(&ModulePCTable[i].Start[M.Idx(P)]);
              //*P dereference value is the x times of the edge.
            }
        }
      }
    }
...
void TracePC::IterateCoveredFunctions(CallBack CB, struct CoverageInfo &CI) {
  for (size_t i = 0; i < NumPCTables; i++) {
    auto &M = ModulePCTable[i];
    ...
    for (auto NextFE = M.Start; NextFE < M.Stop;) {
      ...
      CB(FE, NextFE, CI.ObservedFuncs[FE->PC]);
    }
  }
}
...
void TracePC::SetFocusFunction(const std::string &FuncName) {
  ...
  for (size_t M = 0; M < NumModules; M++) {
    ...
    for (size_t I = 0; I < N; I++) {
      ...
      Printf("INFO: Focus function is set to '%s'\n", Name.c_str());
      // FocusFunctionCounterPtr = Modules[M].Start() + I;
      FocusFunctionsCounterPtr.push_back(Modules[M].Start() + I);
      // return;
    }
  }
  return;
...
size_t TracePC::ObservedFocusFunctions() {
  std::vector<uint8_t *> observed;
  size_t SumAward = 0;
  size_t L = FocusFunctionsCounterPtr.size();
  size_t i = 0;
  for (auto *counterPtr : FocusFunctionsCounterPtr) {
    if (counterPtr && *counterPtr) {
      observed.push_back(counterPtr);
      SumAward += (L - i);
    }
    i++;
  }
...
void TracePC::GetFuncFreqsUncoveredInfo(struct CoverageInfo &CI) {
  ...
  auto CoveredFunctionCallback = [&](const PCTableEntry *First,
    ...
    if (Counter == 0)
      return;
    if (CI.LastObservedFuncs[First->PC] == Counter)
      return;
    CI.LastObservedFuncs[First->PC] = Counter;
    ...
    size_t UnSize = 0, CSize = 0;

    for (auto TE = First; TE < Last; TE++) {
      if (!CI.ObservedPCs.count(TE)) {
    ...
    auto TmpFuncInfo = std::unique_ptr<FuncInfo>(new FuncInfo(VisualizePC, (size_t)Counter, UnSize, CSize));
    auto It = std::find_if(CI.FuncsInfo.begin(), CI.FuncsInfo.end(), [&TmpFuncInfo](FuncInfo F) {
      return F.Id == TmpFuncInfo->Id;
    });

    if (It != CI.FuncsInfo.end()) {
      It->Hits = Counter;
      It->UncoverSize = UnSize;
      It->CoveredSize = CSize;
    } else {
      CI.FuncsInfo.push_back(*TmpFuncInfo);
    }
  };

  IterateCoveredFunctions(CoveredFunctionCallback, CI);
}

void TracePC::PrintCoverage(bool PrintAllCounters, struct CoverageInfo &CI) {
...
ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY void __sanitizer_weak_hook_memcmp(void *caller_pc, const void *s1,
...
ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY void __sanitizer_weak_hook_strncmp(void *caller_pc, const char *s1,
...
ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY void __sanitizer_weak_hook_strcmp(void *caller_pc, const char *s1,
...
ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY void __sanitizer_weak_hook_strncasecmp(void *called_pc, const char *s1,
                                                                                        const char *s2, size_t n, int result) {
  if (!fuzzer::RunningUserCallback)
    return;
...
ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY void __sanitizer_weak_hook_strcasecmp(void *called_pc, const char *s1,
                                                                                       const char *s2, int result) {
  if (!fuzzer::RunningUserCallback)
    return;
...
ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY void __sanitizer_weak_hook_strstr(void *called_pc, const char *s1,
...
ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY void __sanitizer_weak_hook_strcasestr(void *called_pc, const char *s1,
                                                                                       const char *s2, char *result) {
  if (!fuzzer::RunningUserCallback)
    return;
  fuzzer::TPC.MMT.Add(reinterpret_cast<const uint8_t *>(s2), strlen(s2));
...
ATTRIBUTE_INTERFACE ATTRIBUTE_NO_SANITIZE_MEMORY void __sanitizer_weak_hook_memmem(void *called_pc, const void *s1, size_t len1,
